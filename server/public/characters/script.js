...
async function changeChara(){
	...
	const res = await fetch(`../api/v1/characters/${currentData}/${selectedGroup}/${selectedVal}`, {
		method: 'GET',
...
function loadCharacters(charactersOfType){
	let name = '';
	for(let key in charactersOfType){
		let group = $('<optgroup label="' + key.toUpperCase() + '"/>');
		for(let i = 0; i< charactersOfType[key].length; i++){
			name = charactersOfType[key][i];
			$('<option value="' + name + '"/>').html(name).appendTo(group);
		}
